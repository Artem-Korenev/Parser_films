...
class FilmsSpiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        ):  # переходим по гиперссылкам
            href = link.css("::attr(href)").get()
            if href:
                full_url = response.urljoin(href)
                if (
                    full_url not in self.visited_urls
                ):  # если не заходили ранее по этой ссылке, то переходим и запоминаем ее
                    self.visited_urls.add(full_url)
                    yield response.follow(href, self.parse)
        infobox = response.xpath(
            '//table[contains(@class, "infobox")]'
        )  # Если больше нет ссылок, ищем сводную таблицу на странице со всеми данными
        if infobox:
            title = response.xpath(
                '//h1[@id="firstHeading"]/span/text() | //h1[@id="firstHeading"]/i/text()'
            ).getall()  # получение названия
            genre = response.css(
            ...
            director = response.css(
                'th:contains("Режиссёр") + td ::text, th:contains("Режиссёры") + td ::text'
            ).getall()  # получение режиссера
            director = self.clean_text(director)
            country = response.css(
                'th:contains("Страна") + td ::text, th:contains("Страны") + td ::text'
            ).getall()  # получение страны
            country = self.clean_text(country)
            year = response.css(
                'th:contains("Год") + td ::text, '
                'th:contains("Дата выхода") + td ::text, '
                'th:contains("Премьерный показ") + td ::text, '
                'th:contains("Публикация") + td ::text'
            ).getall()  # получение года
